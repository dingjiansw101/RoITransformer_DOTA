    ...
    for im_info, data_batch in test_data:
        ...
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, boxes_all, data_dict_all)):
            ...
            for j in range(1, imdb.num_classes):
                ...
                # TODO: test the accuracy of gpu nms
                # gpu_nms = poly_gpu_nms_wrapper(0.3, 0)
                # if len(cls_quadrangle_dets) > 0:
                #     keep = gpu_nms(cls_quadrangle_dets.astype(np.float32))
                # else:
                #     # keep = []
                #     keep = py_cpu_nms_poly(cls_quadrangle_dets, 0.3)
                # all_boxes[j][idx+delta] = cls_quadrangle_dets[keep, :]
                all_boxes[j][idx+delta] = cls_quadrangle_dets # for debug
                # pdb.set_trace()
            if max_per_image > 0:
                image_scores = np.hstack([all_boxes[j][idx+delta][:, -1]
                                          for j in range(1, imdb.num_classes)])
...
def im_detect_rotbox_Rroi(predictor, data_batch, data_names, scales, cfg):
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        if cfg.TEST.HAS_RPN:
            ...
            rois = data_dict['Rrois_rois'].asnumpy().reshape((-1, 6))[:, 1:]
        im_shape = data_dict['data'].shape
