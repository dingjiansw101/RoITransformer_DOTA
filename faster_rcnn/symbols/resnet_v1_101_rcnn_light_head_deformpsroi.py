...
class resnet_v1_101_rcnn_light_head_deformpsroi(Symbol):
    def __init__(self):
        """
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            if cfg.TRAIN.CXX_PROPOSAL:
                ...
                rois = mx.sym.Custom(
                    ...
                    rpn_pre_nms_top_n=cfg.TRAIN.RPN_PRE_NMS_TOP_N, rpn_post_nms_top_n=cfg.TRAIN.RPN_POST_NMS_TOP_N,
                    threshold=cfg.TRAIN.RPN_NMS_THRESH, rpn_min_size=cfg.TRAIN.RPN_MIN_SIZE)
            # print 'in get_symbol, after proposal'
            # pdb.set_trace()
            ...
            # print 'in get_symbol, after proposal_target'
            # pdb.set_trace()
        else:
            # ROI Proposal
            rpn_cls_score_reshape = mx.sym.Reshape(
        ...
        #
        # roi_pool = mx.contrib.symbol.P(
        #     name='roi_pool', data=conv_thin_feat, rois=rois, pooled_size=(7, 7), spatial_scale=0.0625)
        #
        ...
        # pdb.set_trace()
        # roi_pool = mx.contrib.sym.PSROIALIGNAVEPooling(name='psroialignave', data=conv_thin_feat, rois=rois, spatial_scale=0.065,
        #                                       group_size=7, pooled_size=7,
        #                                       output_dim=10)

        # deformable psroipooling
        # pdb.set_trace()
        offset_t = mx.contrib.sym.DeformablePSROIPooling(name='offset_t', data=conv_thin_feat, rois=rois, group_size=7, pooled_size=7,
                                                         sample_per_part=4, no_trans=True, part_size=7, output_dim=10, spatial_scale=0.0625)
        ...
        deformable_roi_pool = mx.contrib.sym.DeformablePSROIPooling(name='deformable_roi_pool', data=conv_thin_feat, rois=rois,
                                                                    trans=offset_reshape, group_size=7, pooled_size=7, sample_per_part=4,
                                                                    no_trans=False, part_size=7, output_dim=10, spatial_scale=0.0625, trans_std=0.1)

        # 2 fc
        # fc_new_1 = mx.symbol.FullyConnected(name='fc_new_1', data=roi_pool, num_hidden=1024)
        fc_new_1 = mx.symbol.FullyConnected(name='fc_new_1', data=deformable_roi_pool, num_hidden=2048)
        ...
        bbox_pred = mx.symbol.FullyConnected(name='bbox_pred', data=fc_new_1_relu, num_hidden=num_reg_classes * 5)
