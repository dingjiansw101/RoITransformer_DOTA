...
import pdb
DEBUG = False
class resnet_v1_101_rcnn_light_head_RoITransformer(Symbol):
    def __init__(self):
        """
        ...
        self.shared_param_list = ['conv_new_1', 'conv_new_2', 'conv_new_3', 'conv_new_4']
    ...
    def get_light_head(self, data, mid_num_filter=256, suffix='separable'):
        # mid_num_filter=256
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
                                                                  op_type='proposal_target_rotbox',
                                                                  num_classes=num_reg_classes,
                                                                  batch_images=cfg.TRAIN.BATCH_IMAGES,
                                                                  batch_rois=cfg.TRAIN.BATCH_ROIS,
                                                                  cfg=cPickle.dumps(cfg),
                                                                  fg_class_agnostic=True,
        ...
        # largeseparable
        conv_thin_feat = self.get_light_head(data=relu1, mid_num_filter=256)
        roi_pool = mx.contrib.sym.PSROIALIGNAVEPooling(name='psroialign', data=conv_thin_feat, rois=rois, group_size=7, pooled_size=7,
                                                       output_dim=10, spatial_scale=0.0625, sampling_ratio=4)
        # RRoI Learner
        ...
        cls_prob = mx.sym.Reshape(data=cls_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_reg_classes),
        ...
        if is_train:
            if cfg.TRAIN.ENABLE_OHEM:
                ...
                cls_prob_loss = mx.sym.SoftmaxOutput(name='cls_prob_loss', data=cls_score, label=labels_ohem,
                                                normalization='valid', use_ignore=True, ignore_label=-1)
            ...
            cls_prob_loss = mx.sym.Reshape(data=cls_prob_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_reg_classes),
                                      name='cls_prob_loss_reshape')
            bbox_loss = mx.sym.Reshape(data=bbox_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 5 * num_reg_classes),
        ...
        if is_train:
            Rroi_aux_dict = {
                ...
                'threshold': cfg.TRAIN.RRoI_NMS_THRESH, 'min_size': cfg.TRAIN.RRoI_MIN_SIZE,
                'cfg': cPickle.dumps(cfg)
            }
            Rrois, Rrois_elarge = mx.symbol.Custom(**dict(Rroi_arg_dict.items() + Rroi_aux_dict.items()))

            # rotated proposal target
            Rrois, Rrois_elarge_gt_ag, Rroi_label, Rroi_bbox_target, Rroi_bbox_weight = mx.symbol.Custom(Rrois=Rrois,
                                                                                              gt_boxes=gt_boxes_reshape,
                                                                                              op_type='RRoI_target_rotbox_v2',
                                                                                              num_classes=Rroi_num_reg_classes,
                                                                                              batch_images=cfg.TRAIN.BATCH_IMAGES,
                                                                                              batch_rois=cfg.TRAIN.RRoI_BATCH_ROIS,
                                                                                              cfg=cPickle.dumps(cfg),
            ...
            Rroi_aux_dict = {
                ...
                'threshold': cfg.TEST.RRoI_NMS_THRESH, 'min_size': cfg.TEST.RRoI_MIN_SIZE,
                'cfg': cPickle.dumps(cfg)
            }
            Rrois, Rrois_elarge = mx.symbol.Custom(**dict(Rroi_arg_dict.items() + Rroi_aux_dict.items()))
        if is_train:
            rotated_pool = mx.contrib.sym.PSROIALIGNAVERotatedPooling(name='psroialign_rotated', data=conv_thin_feat, rois=Rrois_elarge_gt_ag, group_size=7, pooled_size=7,
                                                       output_dim=10, spatial_scale=0.0625, sampling_ratio=4)
        else:
            rotated_pool = mx.contrib.sym.PSROIALIGNAVERotatedPooling(name='psroialign_rotated', data=conv_thin_feat, rois=Rrois_elarge, group_size=7, pooled_size=7,
                                                       output_dim=10, spatial_scale=0.0625, sampling_ratio=4)
        ...
        # 2 fc
        # cls_score/bbox_pred
        Rroi_cls_score = mx.symbol.FullyConnected(name='Rroi_cls_score', data=fc_new_3_relu,
                                                     num_hidden=num_classes)
        Rroi_bbox_pred = mx.symbol.FullyConnected(name='Rroi_bbox_pred', data=fc_new_3_relu,
                                                     num_hidden=Rroi_num_reg_classes * 5)

        if is_train:
            if cfg.TRAIN.RRoI_ENABLE_OHEM:
                # turn off ohem
                Rroi_labels_ohem, Rroi_bbox_weights_ohem = mx.sym.Custom(name='Rroi_ohem',
                                                                               op_type='BoxAnnotatorOHEM',
                                                                               num_classes=num_classes,
                                                                               num_reg_classes=Rroi_num_reg_classes,
                                                                               roi_per_img=cfg.TRAIN.RRoI_BATCH_ROIS_OHEM,
                                                                               cls_score=Rroi_cls_score,
                                                                               bbox_pred=Rroi_bbox_pred,
                                                                               labels=Rroi_label,
                                                                               bbox_targets=Rroi_bbox_target,
                                                                               bbox_weights=Rroi_bbox_weight)
                Rroi_cls_prob = mx.sym.SoftmaxOutput(name='Rroi_cls_prob', data=Rroi_cls_score,
                                                        label=Rroi_labels_ohem, normalization='valid',
                                                        use_ignore=True, ignore_label=-1)
                Rroi_bbox_loss_ = Rroi_bbox_weights_ohem * mx.sym.smooth_l1(name='Rroi_bbox_loss_', scalar=1.0,
                                                                                  data=(
                                                                                  Rroi_bbox_pred - Rroi_bbox_target))
                Rroi_bbox_loss = mx.sym.MakeLoss(name='Rroi_bbox_loss', data=Rroi_bbox_loss_,
                                                    grad_scale=1.0 / cfg.TRAIN.RRoI_BATCH_ROIS_OHEM)
                Rroi_rcnn_label = Rroi_labels_ohem
            else:

                Rroi_cls_prob = mx.sym.SoftmaxOutput(name='Rroi_cls_prob', data=Rroi_cls_score,
                                                        label=Rroi_label, normalization='valid')
                Rroi_bbox_loss_ = Rroi_bbox_weight * mx.sym.smooth_l1(name='Rroi_bbox_loss_', scalar=1.0,
                                                                            data=(
                                                                            Rroi_bbox_pred - Rroi_bbox_target))
                Rroi_bbox_loss = mx.sym.MakeLoss(name='Rroi_bbox_loss', data=Rroi_bbox_loss_,
                                                    grad_scale=1.0 / cfg.TRAIN.RRoI_BATCH_ROIS)
                Rroi_rcnn_label = Rroi_label

            # reshape output
            Rroi_rcnn_label = mx.sym.Reshape(data=Rroi_rcnn_label, shape=(cfg.TRAIN.BATCH_IMAGES, -1),
                                                name='Rroi_label_reshape')
            Rroi_cls_prob = mx.sym.Reshape(data=Rroi_cls_prob, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_classes),
                                              name='Rroi_cls_prob_reshape')
            Rroi_bbox_loss = mx.sym.Reshape(data=Rroi_bbox_loss,
                                               shape=(cfg.TRAIN.BATCH_IMAGES, -1, 5 * Rroi_num_reg_classes),
                                               name='Rroi_bbox_loss_reshape')
            group = mx.sym.Group([rpn_cls_prob, rpn_bbox_loss, cls_prob_loss, bbox_loss, mx.sym.BlockGrad(rcnn_label),
                                  Rroi_cls_prob, Rroi_bbox_loss, mx.sym.BlockGrad(Rroi_rcnn_label)])
        else:
            Rroi_cls_prob = mx.sym.SoftmaxActivation(name='Rroi_cls_prob', data=Rroi_cls_score)
            Rroi_cls_prob = mx.sym.Reshape(data=Rroi_cls_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_classes),
                                              name='Rroi_cls_prob_reshape')
            Rroi_bbox_pred = mx.sym.Reshape(data=Rroi_bbox_pred,
                                               shape=(cfg.TEST.BATCH_IMAGES, -1, 5 * Rroi_num_reg_classes),
                                               name='Rroi_bbox_pred_reshape')
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        print 'should not in this function'
        pdb.set_trace()
        # input init
        if is_train:
            ...
            bbox_target = mx.symbol.Reshape(data=bbox_target, shape=(-1, 5 * num_classes), name='bbox_target_reshape')
            bbox_weight = mx.symbol.Reshape(data=bbox_weight, shape=(-1, 5 * num_classes), name='bbox_weight_reshape')
        else:
            data = mx.sym.Variable(name="data")
            rois = mx.symbol.Variable(name='rois')
            # reshape input
        ...
        roi_pool = mx.contrib.sym.PSROIPooling(name='psroipooling', data=conv_thin_feat, rois=rois, group_size=7, pooled_size=7,
        ...
        # cls_score/bbox_pred
        cls_score = mx.symbol.FullyConnected(name='cls_score', data=roi_pool, num_hidden=num_classes)
        bbox_pred = mx.symbol.FullyConnected(name='bbox_pred', data=roi_pool, num_hidden=num_reg_classes * 5)
