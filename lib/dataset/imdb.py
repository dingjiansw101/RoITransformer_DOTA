...
from bbox.bbox_transform import bbox_overlaps, get_best_begin_point_wrapp
...
class IMDB(object):
    ...
    def append_rotated_images(self, roidb):
        # TODO: add rotated augmentation here
        pass
    ...
    def append_flipped_images_poly(self, roidb):
        """
        append flipped images to an roidb
        flip boxes coordinates, images will be actually flipped when loading into network
        :param roidb: [image_index]['boxes', 'gt_classes', 'gt_overlaps', 'flipped']
        :return: roidb: [image_index]['boxes', 'gt_classes', 'gt_overlaps', 'flipped']
        """
        # flipp will make the center no longer the first point of the baseball-diamond
        # assert "the flipp is forbidded, recommand to use rotation"
        print 'append flipped images to roidb'
        print 'num_images:', self.num_images, 'len roidb:', len(roidb)
        assert self.num_images == len(roidb)
        for i in range(self.num_images):
            roi_rec = roidb[i]
            boxes = roi_rec['boxes'].copy()
            ...
            for idx, item in enumerate(boxes):
                boxes[idx, :] = get_best_begin_point_wrapp(boxes[idx, :])
