...
from utils.image import get_image, tensor_vstack,get_test_image
from generate_anchor import generate_anchors
from bbox.bbox_transform import bbox_overlaps, bbox_transform, bbox_poly2hbb
import pdb
...
def get_rpn_testbatch(roidb, cfg):
    ...
    imgs, roidb = get_test_image(roidb, cfg)
    im_array = imgs
...
def assign_pyramid_anchor_poly(feat_shapes, gt_boxes, im_info, cfg, feat_strides=(4, 8, 16, 32, 64),
                          scales=(8,), ratios=(0.5, 1, 2), allowed_border=0, balance_scale_bg=False,):
    """
    assign ground truth boxes to anchor positions
    :param feat_shapes: infer output shape
    ...
    gt_boxes = bbox_poly2hbb(gt_boxes)
...
def assign_anchor_poly(feat_shape, gt_boxes, im_info, cfg, feat_stride=16,
                       scales=(8, 16, 32), ratios=(0.5, 1, 2), allowed_border=0):
    """
    assign ground truth boxes to anchor positions
    :param feat_shape: infer output shape
