...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Splitting the DOTA images into small patches')
    parser.add_argument('--dataset',dest='dataset',
                        help='path of dataset',
                        type=str,required=True)
    parser.add_argument('--dest', dest='dest',
                        help='path of splitted dataset',
                        type=str,required=True)
    parser.add_argument('--scale', dest='scale',
                        help='scale for splitting',
                        type=float,default=1.0)
    parser.add_argument('--pad', dest='pad',
                        help='pad image or not',
                        default=True, type=bool)
    args = parser.parse_args()

    # dest = os.path.join(args.dest,'scale{}'.format(args.scale))
    print('args.dataset:', args.dataset)
    print('args.dest:', args.dest)
    print('args.padding:', args.pad)
    split = splitbase(args.dataset,
                      args.dest, padding=args.pad)
