...
from rpn.rpn import get_rpn_testbatch, get_rpn_batch, get_rpn_batch_poly, assign_pyramid_anchor, assign_pyramid_anchor_poly
...
def par_assign_anchor_wrapper_poly(cfg, iroidb, feat_sym, feat_strides, anchor_scales, anchor_ratios, allowed_border):
    # get testing data for multigpu
    data, rpn_label = get_rpn_batch_poly(iroidb, cfg)
    data_shape = {k: v.shape for k, v in data.items()}
    del data_shape['im_info']
    # pdb.set_trace()
    # add gt_boxes to data for e2e
    ...
    feat_shape = [y[1] for y in [x.infer_shape(**data_shape) for x in feat_sym]]
    label = assign_pyramid_anchor_poly(feat_shape, rpn_label['gt_boxes'], data['im_info'], cfg,
                                  feat_strides, anchor_scales, anchor_ratios, allowed_border)
    return {'data': data, 'label': label}
...
class PyramidAnchorIterator(mx.io.DataIter):
    ...
    def get_batch_parallel(self):
        ...
        for idx, islice in enumerate(slices):
            #print(range(islice.start, islice.stop))
...
class PyramidAnchorIterator_poly(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        :return: AnchorLoader
        """
        super(PyramidAnchorIterator_poly, self).__init__()
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        feat_shape = [y[1] for y in [x.infer_shape(**max_shapes) for x in self.feat_sym]]
        label = assign_pyramid_anchor_poly(feat_shape, np.zeros((0, 9)), im_info, self.cfg,
                                      self.feat_strides, self.anchor_scales, self.anchor_ratios, self.allowed_border)
        label = [label[k] for k in self.label_name]
        label_shape = [(k, tuple([input_batch_size] + list(v.shape[1:]))) for k, v in zip(self.label_name, label)]
    ...
    def get_batch_parallel(self):
        ...
        for idx, islice in enumerate(slices):
            #print(range(islice.start, islice.stop))
            iroidb = [roidb[i] for i in range(islice.start, islice.stop)]
            rst.append(par_assign_anchor_wrapper_poly(self.cfg, iroidb, self.feat_sym, self.feat_strides, self.anchor_scales,
                                                 self.anchor_ratios, self.allowed_border))
