...
def do_checkpoint_Rroi(prefix, means, stds, Rroi_means, Rroi_stds):
    def _callback(iter_no, sym, arg, aux):
        # pdb.set_trace()
        arg['bbox_pred_weight_test'] = (arg['bbox_pred_weight'].T * mx.nd.array(stds)).T
        arg['bbox_pred_bias_test'] = arg['bbox_pred_bias'] * mx.nd.array(stds) + mx.nd.array(means)
        # params for Rroi regression
        arg['Rroi_bbox_pred_weight_test'] = (arg['Rroi_bbox_pred_weight'].T * mx.nd.array(Rroi_stds)).T
        arg['Rroi_bbox_pred_bias_test'] = arg['Rroi_bbox_pred_bias'] * mx.nd.array(Rroi_stds) + mx.nd.array(Rroi_means)

        mx.model.save_checkpoint(prefix, iter_no + 1, sym, arg, aux)
        arg.pop('bbox_pred_weight_test')
        arg.pop('bbox_pred_bias_test')
        arg.pop('Rroi_bbox_pred_weight_test')
        arg.pop('Rroi_bbox_pred_bias_test')
    return _callback