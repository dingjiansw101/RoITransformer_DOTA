...
def get_Rroi_names(cfg):
    pred, label = get_rcnn_names(cfg)
    pred.append('Rroi_rcnn_cls_prob')
    pred.append('Rroi_rcnn_bbox_loss')

    if cfg.TRAIN.ENABLE_OHEM or cfg.TRAIN.END2END:
        pred.append('Rroi_rcnn_label')
...
class RCNNFGFraction(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(RCNNFGFraction, self).__init__('RRoI Proposal FG Fraction')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_Rroi_names(cfg)

    def update(self, labels, preds):
        pred = preds[self.pred.index('Rroi_rcnn_cls_prob')]
        if self.ohem or self.e2e:
            label = preds[self.pred.index('Rroi_rcnn_label')]
        else:
            label = labels[self.label.index('Rroi_rcnn_label')]
        num_classes = pred.shape[-1]
        # selection of ground truth label is different from softmax or sigmoid classifier
        label = label.asnumpy().reshape(-1, ).astype('int32')
        fg_inds = np.where(label > 0)[0]
...
class RRoIRCNNFGAccuracy(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(RRoIRCNNFGAccuracy, self).__init__('RRoIR-CNN FG Accuracy')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_Rroi_names(cfg)
...
class RRoIAccMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(RRoIAccMetric, self).__init__('RRoIAcc')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_Rroi_names(cfg)
...
class RRoIRCNNLogLossMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(RRoIRCNNLogLossMetric, self).__init__('RRoIRCNNLogLoss')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_Rroi_names(cfg)
...
class RRoIRCNNL1LossMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(RRoIRCNNL1LossMetric, self).__init__('RRoIRCNNL1Loss')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_Rroi_names(cfg)
    ...
    def update(self, labels, preds):
        # pdb.set_trace()
        bbox_loss = preds[self.pred.index('Rroi_rcnn_bbox_loss')].asnumpy()
        if self.ohem:
            label = preds[self.pred.index('Rroi_rcnn_label')].asnumpy()
        else:
            if self.e2e:
                label = preds[self.pred.index('Rroi_rcnn_label')].asnumpy()
            else:
                label = labels[self.label.index('Rroi_rcnn_label')].asnumpy()
        # pdb.set_trace()
        # calculate num_inst (average on those kept anchors)
        num_inst = np.sum(label != -1)
...
class STLogLossMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(STLogLossMetric, self).__init__('STLogLoss')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_rcnn_names(cfg)
...
class STL1LossMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(STL1LossMetric, self).__init__('STL1Loss')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_rcnn_names(cfg)
        # add st loss here
        self.pred.append('st_loss')

    def update(self, labels, preds):
        st_loss = preds[self.pred.index('st_loss')].asnumpy()

        label = preds[-2].asnumpy()
        # pdb.set_trace()
        num_inst = np.sum(label != 0)

        self.sum_metric += np.sum(st_loss)
        self.num_inst += num_inst