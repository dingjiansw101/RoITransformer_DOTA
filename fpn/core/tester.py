...
from bbox.bbox_transform import bbox_pred, dbbox_pred, clip_boxes, clip_polys, dbbox_transform2_inv_warp, dbbox_transform2_warp
from bbox.bbox_transform import polygonToRotRectangle_batch, RotBox2Polys_multi_class
from bbox.bbox_transform import dbboxtransform3_inv_warp, xyhs2polys, xyhs2polys_muli_class
from bbox.bbox_transform import dbbox_transform2_inv, dbbox_transform2_inv_new
...
# def im_detect_poly_abstract(predictor, data_batch, data_names, scales, cfg):
#     """
#     The function merge poly and rotbox encoding
#     :param predictor:
#     :param data_batch:
#     :param data_names:
#     :param scales:
...
#         # post processing
#         # pdb.set_trace()
#         if cfg.network.BOXENCODING == 'rotbox':
#
#         pred_boxes = box_decode(rois, bbox_deltas)
#         pred_boxes = clip_polys(pred_boxes, im_shape[-2:])
#
#         # we used scaled image & roi to train, so it is necessary to transform them back
#         pred_boxes = pred_boxes / scale
#
...
#
#     return scores_all, pred_boxes_all, data_dict_all
def im_detect_poly(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        ...
        # post processing
        # pdb.set_trace()
        pred_boxes = dbbox_pred(rois, bbox_deltas)
        pred_boxes = clip_polys(pred_boxes, im_shape[-2:])
...
def im_detect_rotbox(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        ...
        pred_boxes = dbbox_transform2_inv_warp(rois, bbox_deltas)
        pred_polys = RotBox2Polys_multi_class(pred_boxes)
        pred_polys = clip_polys(pred_polys, im_shape[-2:])

        pred_polys = pred_polys / scale

        scores_all.append(scores)
        pred_boxes_all.append(pred_polys)

    return scores_all, pred_boxes_all, data_dict_all

def im_detect_rotbox_Rroi(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        if cfg.TEST.HAS_RPN:
            # rois = output['rois_output'].asnumpy()[:, 1:]
            rois = output['Rrois_output'].asnumpy()[:, 1:]
        else:
            rois = data_dict['rois'].asnumpy().reshape((-1, 6))[:, 1:]
        im_shape = data_dict['data'].shape

        # save output
        scores = output['Rroi_cls_prob_reshape_output'].asnumpy()[0]
        bbox_deltas = output['Rroi_bbox_pred_reshape_output'].asnumpy()[0]

        # DEBUG = True
        if DEBUG:
            bbox_deltas = np.zeros_like(bbox_deltas)

        # pred_boxes = dbbox_transform2_inv(rois, bbox_deltas)
        # pdb.set_trace()
        pred_boxes = dbbox_transform2_inv_new(rois, bbox_deltas, np.pi/2.)
...
def im_detect_xyhs(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        ...
        # pred_boxes = dbbox_transform2_inv_warp(rois, bbox_deltas)
        pred_boxes = dbboxtransform3_inv_warp(rois, bbox_deltas)
        # pred_polys = RotBox2Polys_multi_class(pred_boxes)
        pred_polys = xyhs2polys_muli_class(pred_boxes)
        pred_polys = clip_polys(pred_polys, im_shape[-2:])
...
def detect_at_single_scale(predictor, data_names, imdb, test_data, cfg, thresh, vis, all_boxes_single_scale, logger):
    ...
    for im_info, data_batch in test_data:
        ...
        print 'scales',scales.__len__()
        print 'boxes_all', boxes_all.__len__()
        print 'data_dict_all', data_dict_all.__len__()
...
def detect_at_single_scale_poly(predictor, data_names, imdb, test_data, cfg, thresh, vis, all_boxes_single_scale, logger):
    idx = 0
    data_time, net_time, post_time = 0.0, 0.0, 0.0
    t = time.time()
    print test_data
...
def pred_eval_poly(predictor, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    """
    ...
    print "In tester.py line 147 num_images:",num_images
    ...
    for test_scale_index, test_scale in enumerate(cfg.TEST_SCALES):
        ...
        detect_at_single_scale_poly(predictor, data_names, imdb, test_data, cfg, thresh, vis, all_boxes_single_scale, logger)
    ...
    for test_scale_index, test_scale in enumerate(cfg.TEST_SCALES):
        ...
        if os.path.exists(det_file_single_scale):
            ...
            for idx_class in range(1, imdb.num_classes):
                ...
                #print "ALL_BOXES:",idx_class, all_boxes[idx_class].__len__()
    ...
    #             print all_boxes[idx_class][idx_im]
...
def pred_eval_poly_multiscale(prefix, predictor, test_data, imdb, cfg, vis=False, draw=False,
    ...
    for im_info, data_batch in test_data:
        ...
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, boxes_all, data_dict_all)):
            ...
            if draw:
                ...
                boxes_this_image = [[]] + [all_boxes[j][idx + delta] for j in range(1, imdb.num_classes)]
                im = draw_all_poly_detection(data_dict['data'].asnumpy(), boxes_this_image, imdb.classes, scales[delta], cfg, threshold=0.2)
                print path
                cv2.imwrite(path, im)
...
def pred_eval_dota_poly(predictor, test_data, imdb, cfg, vis=False, draw=False,
                          thresh=1e-3, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
...
def pred_eval_dota_rotbox(predictor, test_data, imdb, cfg, vis=False, draw=False,
                          thresh=1e-3, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
    ...
    for im_info, data_batch in test_data:
        ...
        scales = [iim_info[0, 2] for iim_info in im_info]
        # scores_all, boxes_all, data_dict_all= im_detect_poly(predictor, data_batch, data_names, scales, cfg)
        scores_all, boxes_all, data_dict_all = im_detect_rotbox(predictor, data_batch, data_names, scales, cfg)
        # pdb.set_trace()
        t2 = time.time() - t
        t = time.time()
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, boxes_all, data_dict_all)):
...
def pred_eval_dota_rotbox_Rroi(predictor, test_data, imdb, cfg, vis=False, draw=False,
                          thresh=1e-3, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
    ...
    for im_info, data_batch in test_data:
        ...
        scales = [iim_info[0, 2] for iim_info in im_info]
        # scores_all, boxes_all, data_dict_all= im_detect_poly(predictor, data_batch, data_names, scales, cfg)
        scores_all, boxes_all, data_dict_all = im_detect_rotbox_Rroi(predictor, data_batch, data_names, scales, cfg)
        # pdb.set_trace()
        t2 = time.time() - t
        t = time.time()
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, boxes_all, data_dict_all)):
            ...
            for j in range(1, imdb.num_classes):
                indexes = np.where(scores[:, j] > thresh)[0]
                cls_scores = scores[indexes, j, np.newaxis]
                cls_boxes = boxes[indexes, 8:16] if cfg.network.RRoI_CLASS_AGNOSTIC else boxes[indexes, j * 8:(j + 1) * 8]
                cls_quadrangle_dets = np.hstack((cls_boxes, cls_scores))
                # keep = nms(cls_dets)
                keep = py_cpu_nms_poly(cls_quadrangle_dets, 0.3)
                # pdb.set_trace()
...
def pred_eval_dota_wyhs(predictor, test_data, imdb, cfg, vis=False, draw=False,
                          thresh=1e-3, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
    ...
    for im_info, data_batch in test_data:
        ...
        scales = [iim_info[0, 2] for iim_info in im_info]
        # scores_all, boxes_all, data_dict_all= im_detect_poly(predictor, data_batch, data_names, scales, cfg)
        scores_all, boxes_all, data_dict_all = im_detect_xyhs(predictor, data_batch, data_names, scales, cfg)
        # pdb.set_trace()
        t2 = time.time() - t
        t = time.time()
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, boxes_all, data_dict_all)):
