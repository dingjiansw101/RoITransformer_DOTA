...
from symbols import *
from core.loader import PyramidAnchorIterator_poly
from core import callback, metric
from core.module import MutableModule
from utils.create_logger import create_logger
from utils.load_data import merge_roidb, filter_roidb, load_gt_roidb_poly
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    roidbs = [load_gt_roidb_poly(config.dataset.dataset, image_set, config.dataset.root_path, config.dataset.dataset_path,
    ...
    train_data = PyramidAnchorIterator_poly( feat_sym, roidb, config, batch_size=input_batch_size, shuffle=config.TRAIN.SHUFFLE,
    ...
    for child_metric in [rpn_eval_metric, rpn_cls_metric, rpn_bbox_metric, rpn_fg_metric, eval_fg_metric, eval_metric, cls_metric,
                         bbox_metric, RCNN_proposal_fraction_metric, Rroi_fg_accuracy, Rroi_accuracy, Rroi_cls_metric, Rroi_bbox_metric]:
        eval_metrics.add(child_metric)
    # callback
    batch_end_callback = callback.Speedometer(train_data.batch_size, frequent=args.frequent)
    means = np.tile(np.array(config.TRAIN.BBOX_MEANS), 2 if config.CLASS_AGNOSTIC else config.dataset.NUM_CLASSES)
    stds = np.tile(np.array(config.TRAIN.BBOX_STDS), 2 if config.CLASS_AGNOSTIC else config.dataset.NUM_CLASSES)
    Rroi_means = np.tile(np.array(config.TRAIN.RRoI_BBOX_MEANS), 2 if config.network.RRoI_CLASS_AGNOSTIC else config.dataset.NUM_CLASSES)
    Rroi_stds = np.tile(np.array(config.TRAIN.RRoI_BBOX_STDS), 2 if config.network.RRoI_CLASS_AGNOSTIC else config.dataset.NUM_CLASSES)

    epoch_end_callback = [mx.callback.module_checkpoint(mod, prefix, period=1, save_optimizer_states=True), callback.do_checkpoint_Rroi(prefix, means, stds, Rroi_means, Rroi_stds)]
