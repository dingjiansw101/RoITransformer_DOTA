...
from operator_py.proposal_target_rotbox import *
from operator_py.fpn_roi_pooling import *
from operator_py.box_annotator_ohem import *
from operator_py.fpn_rotated_psroialign import *
from operator_py.fpn_psroipooling_v2 import *
from operator_py.fpn_rotated_roialign import *
from operator_py.RRoI_target_rotbox_v2 import *
from operator_py.RRoIDecoder import *

DEBUG = False

class resnet_v1_101_fpn_rcnn_rotbox_light_head_RoITransformer(Symbol):
    def __init__(self):
        """
        Use __init__ to define parameter network needs
        """
        self.shared_param_list = ['rpn_conv', 'rpn_cls_score', 'rpn_bbox_pred', 'conv_new_1', 'conv_new_2', 'conv_new_3', 'conv_new_4']
    ...
    def get_light_head(self, data, suffix):
        # mid_num_filter=256
        mid_num_filter=64 # for s
        conv_new_1 = mx.sym.Convolution(data=data, kernel=(15, 1), pad=(7, 0), num_filter=mid_num_filter, name="conv_new_1" + suffix,
                                        weight=self.shared_param_dict['conv_new_1_weight'], bias=self.shared_param_dict['conv_new_1_bias'], lr_mult=3.0)

        relu_new_1 = mx.sym.Activation(data=conv_new_1, act_type='relu', name='relu1' + suffix)
        conv_new_2 = mx.sym.Convolution(data=relu_new_1, kernel=(1, 15), pad=(0, 7), num_filter=10 * 7 * 7, name="conv_new_2" + suffix,
                                        weight=self.shared_param_dict['conv_new_2_weight'], bias=self.shared_param_dict['conv_new_2_bias'],
                                        lr_mult=3.0)
        relu_new_2 = mx.sym.Activation(data=conv_new_2, act_type='relu', name='relu2' + suffix)
        conv_new_3 = mx.sym.Convolution(data=data, kernel=(1, 15), pad=(0, 7), num_filter=mid_num_filter, name="conv_new_3" + suffix,
                                        weight=self.shared_param_dict['conv_new_3_weight'], bias=self.shared_param_dict['conv_new_3_bias'],
                                        lr_mult=3.0)
        relu_new_3 = mx.sym.Activation(data=conv_new_3, act_type='relu', name='relu3' + suffix)
        conv_new_4 = mx.sym.Convolution(data=relu_new_3, kernel=(15, 1), pad=(7, 0), num_filter=10 * 7 * 7, name="conv_new_4" + suffix,
                                        weight=self.shared_param_dict['conv_new_4_weight'], bias=self.shared_param_dict['conv_new_4_bias'],
                                        lr_mult=3.0)
        relu_new_4 = mx.sym.Activation(data=conv_new_4, act_type='relu', name='relu4' + suffix)
        light_head = mx.symbol.broadcast_add(name='light_head' + suffix, *[relu_new_2, relu_new_4])
        return light_head
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        Rroi_num_reg_classes = (2 if cfg.network.RRoI_CLASS_AGNOSTIC else num_classes)
        ...
        # large separable
        fpn_p2_new = self.get_light_head(fpn_p2, 'p2')
        fpn_p3_new = self.get_light_head(fpn_p3, 'p3')
        fpn_p4_new = self.get_light_head(fpn_p4, 'p4')
        fpn_p5_new = self.get_light_head(fpn_p5, 'p5')
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight \
                = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape, op_type='proposal_target_rotbox', num_classes=num_reg_classes, batch_images=cfg.TRAIN.BATCH_IMAGES,
                                batch_rois=cfg.TRAIN.BATCH_ROIS, cfg=cPickle.dumps(cfg), fg_class_agnostic=True, fg_fraction=cfg.TRAIN.FG_FRACTION)
        else:
            aux_dict = {
                'op_type': 'pyramid_proposal', 'name': 'rois',
                'im_info': im_info, 'feat_stride': tuple(cfg.network.RPN_FEAT_STRIDE),
        ...
        roi_pool = mx.symbol.Custom(data_p2=fpn_p2_new, data_p3=fpn_p3_new, data_p4=fpn_p4_new, data_p5=fpn_p5_new,
                                    rois=rois, op_type='fpn_psroi_pooling_v2', name='fpn_psroi_pooling_v2', pooling_mode='alignave')

        # cls_score/bbox_pred
        cls_score = mx.symbol.FullyConnected(name='cls_score', data=roi_pool, num_hidden=num_reg_classes)
        bbox_pred = mx.symbol.FullyConnected(name='bbox_pred', data=roi_pool, num_hidden=num_reg_classes * 5)

        #------------------------------------------------------------------------------------------------------------------
        # TODO: 1. use output from softmaxcativation for score
        #       2. carefully set shape for bbox_pred, cls_prob
        cls_prob = mx.sym.SoftmaxActivation(name='cls_prob', data=cls_score)
        cls_prob = mx.sym.Reshape(data=cls_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_reg_classes), name='cls_prob_reshape')

        if is_train:
            if cfg.TRAIN.ENABLE_OHEM:
                labels_ohem, bbox_weights_ohem = mx.sym.Custom(name='ohem', op_type='BoxAnnotatorOHEM', num_classes=num_classes,
                                                               num_reg_classes=num_reg_classes, roi_per_img=cfg.TRAIN.BATCH_ROIS_OHEM,
                                                               cls_score=cls_score, bbox_pred=bbox_pred, labels=label,
                                                               bbox_targets=bbox_target, bbox_weights=bbox_weight)
                cls_prob_loss = mx.sym.SoftmaxOutput(name='cls_prob_loss', data=cls_score, label=labels_ohem, normalization='valid', use_ignore=True, ignore_label=-1)
                ...
                cls_prob_loss = mx.sym.SoftmaxOutput(name='cls_prob_loss', data=cls_score, label=label, normalization='valid')
            ...
            cls_prob_loss = mx.sym.Reshape(data=cls_prob_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_reg_classes), name='cls_prob_loss_reshape')
            bbox_loss = mx.sym.Reshape(data=bbox_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 5 * num_reg_classes), name='bbox_loss_reshape')

        # -----------------------------------------------------------------------------------------------------------------------------------
        #
        # shape of bbox_pred (n, 2 * 5), shape of cls_prob (n, 2)
        Rroi_arg_dict = {'rois': rois, 'bbox_pred': bbox_pred, 'cls_prob': cls_prob}
        if is_train:
            Rroi_aux_dict = {
                'op_type': 'RRoIDecoder', 'name': 'Rrois','im_info': im_info,
                'Rroi_pre_nms_top_n': cfg.TRAIN.RRoI_PRE_NMS_TOP_N,
                'Rroi_post_nms_top_n': cfg.TRAIN.RRoI_POST_NMS_TOP_N,
                'threshold': cfg.TRAIN.RRoI_NMS_THRESH, 'min_size': cfg.TRAIN.RRoI_MIN_SIZE, 'cfg': cPickle.dumps(cfg)
            }
            Rrois, Rrois_elarge = mx.symbol.Custom(**dict(Rroi_arg_dict.items() + Rroi_aux_dict.items()))

            # rotated proposal target
            Rrois, Rrois_elarge_gt_ag, Rroi_label, Rroi_bbox_target, Rroi_bbox_weight = mx.symbol.Custom(Rrois=Rrois, gt_boxes=gt_boxes_reshape, op_type='RRoI_target_rotbox_v2',
                                 num_classes=Rroi_num_reg_classes, batch_images=cfg.TRAIN.BATCH_IMAGES,
                                batch_rois=cfg.TRAIN.RRoI_BATCH_ROIS, cfg=cPickle.dumps(cfg),
                                fg_fraction=cfg.TRAIN.RRoI_FG_FRACTION)
        else:
            Rroi_aux_dict = {
                'op_type': 'RRoIDecoder', 'name': 'Rrois','im_info': im_info,
                'Rroi_pre_nms_top_n': cfg.TEST.RRoI_PRE_NMS_TOP_N,
                'Rroi_post_nms_top_n': cfg.TEST.RRoI_POST_NMS_TOP_N,
                'threshold': cfg.TEST.RRoI_NMS_THRESH, 'min_size': cfg.TEST.RRoI_MIN_SIZE, 'cfg': cPickle.dumps(cfg)
            }
            Rrois, Rrois_elarge = mx.symbol.Custom(**dict(Rroi_arg_dict.items() + Rroi_aux_dict.items()))
        if is_train:
            rotated_pool = mx.symbol.Custom(data_p2=fpn_p2_new, data_p3=fpn_p3_new, data_p4=fpn_p4_new, data_p5=fpn_p5_new,
                                        Rrois=Rrois_elarge_gt_ag, op_type='fpn_psroi_rotatedalign', name='fpn_psroi_rotatedalign')
        else:
            rotated_pool = mx.symbol.Custom(data_p2=fpn_p2_new, data_p3=fpn_p3_new, data_p4=fpn_p4_new, data_p5=fpn_p5_new,
                                        Rrois=Rrois_elarge, op_type='fpn_psroi_rotatedalign', name='fpn_psroi_rotatedalign')


        fc_new_3 = mx.symbol.FullyConnected(name='fc_new_3', data=rotated_pool, num_hidden=2048)
        ...
        # 2 fc
        # cls_score/bbox_pred
        Rroi_cls_score = mx.symbol.FullyConnected(name='Rroi_cls_score', data=fc_new_3_relu, num_hidden=num_classes)
        Rroi_bbox_pred = mx.symbol.FullyConnected(name='Rroi_bbox_pred', data=fc_new_3_relu, num_hidden=Rroi_num_reg_classes * 5)

        if is_train:
            if cfg.TRAIN.RRoI_ENABLE_OHEM:
                # turn off ohem for current design
                Rroi_labels_ohem, Rroi_bbox_weights_ohem = mx.sym.Custom(name='Rroi_ohem', op_type='BoxAnnotatorOHEM', num_classes=num_classes,
                                                               num_reg_classes=Rroi_num_reg_classes, roi_per_img=cfg.TRAIN.RRoI_BATCH_ROIS_OHEM,
                                                               cls_score=Rroi_cls_score, bbox_pred=Rroi_bbox_pred, labels=Rroi_label,
                                                               bbox_targets=Rroi_bbox_target, bbox_weights=Rroi_bbox_weight)
                Rroi_cls_prob = mx.sym.SoftmaxOutput(name='Rroi_cls_prob', data=Rroi_cls_score, label=Rroi_labels_ohem, normalization='valid', use_ignore=True, ignore_label=-1)
                Rroi_bbox_loss_ = Rroi_bbox_weights_ohem * mx.sym.smooth_l1(name='Rroi_bbox_loss_', scalar=1.0, data=(Rroi_bbox_pred - Rroi_bbox_target))
                Rroi_bbox_loss = mx.sym.MakeLoss(name='Rroi_bbox_loss', data=Rroi_bbox_loss_, grad_scale=1.0 / cfg.TRAIN.RRoI_BATCH_ROIS_OHEM)
                Rroi_rcnn_label = Rroi_labels_ohem
            else:
                Rroi_cls_prob = mx.sym.SoftmaxOutput(name='Rroi_cls_prob', data=Rroi_cls_score, label=Rroi_label, normalization='valid')
                Rroi_bbox_loss_ = Rroi_bbox_weight * mx.sym.smooth_l1(name='Rroi_bbox_loss_', scalar=1.0, data=(Rroi_bbox_pred - Rroi_bbox_target))
                Rroi_bbox_loss = mx.sym.MakeLoss(name='Rroi_bbox_loss', data=Rroi_bbox_loss_, grad_scale=1.0 / cfg.TRAIN.RRoI_BATCH_ROIS)
                Rroi_rcnn_label = Rroi_label

            # reshape output
            Rroi_rcnn_label = mx.sym.Reshape(data=Rroi_rcnn_label, shape=(cfg.TRAIN.BATCH_IMAGES, -1), name='Rroi_label_reshape')
            Rroi_cls_prob = mx.sym.Reshape(data=Rroi_cls_prob, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_classes), name='Rroi_cls_prob_reshape')
            Rroi_bbox_loss = mx.sym.Reshape(data=Rroi_bbox_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 5 * Rroi_num_reg_classes), name='Rroi_bbox_loss_reshape')
            ...
            group = mx.sym.Group([rpn_cls_output, rpn_bbox_loss, cls_prob_loss, bbox_loss, mx.sym.BlockGrad(rcnn_label), Rroi_cls_prob, Rroi_bbox_loss, mx.sym.BlockGrad(Rroi_rcnn_label)])
        else:
            Rroi_cls_prob = mx.sym.SoftmaxActivation(name='Rroi_cls_prob', data=Rroi_cls_score)
            Rroi_cls_prob = mx.sym.Reshape(data=Rroi_cls_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_classes), name='Rroi_cls_prob_reshape')
            Rroi_bbox_pred = mx.sym.Reshape(data=Rroi_bbox_pred, shape=(cfg.TEST.BATCH_IMAGES, -1, 5 * Rroi_num_reg_classes), name='Rroi_bbox_pred_reshape')

            if DEBUG:
                bbox_pred = mx.sym.Reshape(data=bbox_pred, shape=(cfg.TEST.BATCH_IMAGES, -1, 5 * num_reg_classes),
                                           name='bbox_pred_reshape')
                group = mx.sym.Group([rois, cls_prob, bbox_pred])
            else:
                group = mx.sym.Group([Rrois, Rroi_cls_prob, Rroi_bbox_pred])
    ...
    def init_weight_rcnn(self, cfg, arg_params, aux_params):
        ...
        arg_params['Rroi_cls_score_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['Rroi_cls_score_weight'])
        arg_params['Rroi_cls_score_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['Rroi_cls_score_bias'])
        arg_params['Rroi_bbox_pred_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['Rroi_bbox_pred_weight'])
        arg_params['Rroi_bbox_pred_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['Rroi_bbox_pred_bias'])
