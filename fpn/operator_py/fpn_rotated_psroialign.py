...
class FPNPSROIROTATEDAlignOperator(mx.operator.CustomOp):
        ...
        if is_train:
            ...
            with autograd.train_section():
                for i in range(self.num_strides):
                    ...
                    self.roi_pool[i] = mx.contrib.nd.PSROIALIGNAVERotatedPooling(data=in_data[i],
                                                                  rois=mx.nd.array(rois_p[i], in_data[i].context),
                                                                  group_size=7, pooled_size=7, sampling_ratio=2,
                                                                  output_dim=10,
            ...
            for i in range(self.num_strides):
                # TODO: finish it, and fix the output_dim hard code here
                roi_pool[i] = mx.contrib.nd.PSROIALIGNAVERotatedPooling(data=in_data[i],
                                                              rois=mx.nd.array(rois_p[i], in_data[i].context),
                                                              group_size=7, pooled_size=7, sampling_ratio=2,
                                                              output_dim=10,
                                                              spatial_scale=1.0 / self.feat_strides[i])
...
@mx.operator.register('fpn_psroi_rotatedalign')
class FPNPSROIROTATEDAlignProp(mx.operator.CustomOpProp):
    def __init__(self, feat_strides='(4,8,16,32)', pooled_height='7', pooled_width='7', output_dim='10'):
        super(FPNPSROIROTATEDAlignProp, self).__init__(need_top_grad=True)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return FPNPSROIROTATEDAlignOperator(self.feat_strides, self.pooled_height, self.pooled_width, self.output_dim)
