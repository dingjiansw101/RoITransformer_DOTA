...
inline void PSROIALIGNAVEPoolForward(const Tensor<cpu, 4, DType> &out,
...
template<typename DType>
inline void PSROIALIGNAVEPoolBackwardAcc(const Tensor<cpu, 4, DType> &in_grad,
...
template<>
Operator *CreateOp<cpu>(PSROIALIGNAVEPoolingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new PSROIALIGNAVEPoolingOp<cpu, DType>(param);
  });
  return op;
}

Operator *PSROIALIGNAVEPoolingProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(PSROIALIGNAVEPoolingParam);

MXNET_REGISTER_OP_PROPERTY(_contrib_PSROIALIGNAVEPooling, PSROIALIGNAVEPoolingProp)
...
"the number of region bounding boxes after PSROIALIGNAVEPooling")
...
"in the input data")
.add_arguments(PSROIALIGNAVEPoolingParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
