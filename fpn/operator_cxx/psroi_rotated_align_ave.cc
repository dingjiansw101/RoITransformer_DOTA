...
inline void PSROIALIGNAVERotatedPoolForward(const Tensor<cpu, 4, DType> &out,
...
template<typename DType>
inline void PSROIALIGNAVERotatedPoolBackwardAcc(const Tensor<cpu, 4, DType> &in_grad,
...
template<>
Operator *CreateOp<cpu>(PSROIALIGNAVERotatedPoolingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new PSROIALIGNAVERotatedPoolingOp<cpu, DType>(param);
  });
  return op;
}

Operator *PSROIALIGNAVERotatedPoolingProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(PSROIALIGNAVERotatedPoolingParam);

MXNET_REGISTER_OP_PROPERTY(_contrib_PSROIALIGNAVERotatedPooling, PSROIALIGNAVERotatedPoolingProp)
...
"the number of region bounding boxes after PSROIALIGNAVERotatedPooling")
...
"in the input data")
.add_arguments(PSROIALIGNAVERotatedPoolingParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
