...
*/
#include "./psroi_rotated_align_ave-inl.h"
...
__global__ void PSROIALIGNAVERotatedPoolForwardKernel(
  const int count,
  ...
  CUDA_KERNEL_LOOP(index, count) {
    ...
    DType roi_center_w = offset_bottom_rois[1] * spatial_scale;
    DType roi_center_h = offset_bottom_rois[2] * spatial_scale;
    DType roi_width = offset_bottom_rois[3] * spatial_scale;
    DType roi_height = offset_bottom_rois[4] * spatial_scale;
    // T theta = offset_bottom_rois[5] * M_PI / 180.0;
    DType theta = offset_bottom_rois[5];
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {  // e.g., iy = 0, 1
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        DType y = xx * sinTheta + yy * cosTheta + roi_center_h;
...
template<typename DType>
inline void PSROIALIGNAVERotatedPoolForward(const Tensor<gpu, 4, DType> &out,
  ...
  PSROIALIGNAVERotatedPoolForwardKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
    kBaseThreadNum, 0, stream >> >(
      count, bottom_data, spatial_scale, channels, height, width,
      pooled_height, pooled_width, sampling_ratio, bottom_rois, output_dim_, group_size_, top_data);
  PSROIALIGNAVEROTATEDPOOLING_CUDA_CHECK(cudaPeekAtLastError());
}
...
template <typename DType>
__global__ void PSROIALIGNAVERotatedPoolBackwardAccKernel(
  const int count,
...
template<typename DType>
inline void PSROIALIGNAVERotatedPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
                            ...
                            const int output_dim_,
                            const int group_size_) {
  // LOG(INFO) << "PSROIALIGNAVERotatedPoolBackward";
  ...
  PSROIALIGNAVERotatedPoolBackwardAccKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
    kBaseThreadNum, 0, stream >> >(
...
inline void PSROIALIGNAVERotatedPoolForward(const Tensor<gpu, 4, DType> &out,
                           ...
                           const int output_dim_,
                           const int group_size_) {
  cuda::PSROIALIGNAVERotatedPoolForward(out, data, bbox, spatial_scale, sampling_ratio,output_dim_, group_size_);
}
...
inline void PSROIALIGNAVERotatedPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
                            ...
                            const int output_dim_,
                            const int group_size_) {
  cuda::PSROIALIGNAVERotatedPoolBackwardAcc(in_grad, out_grad, bbox, spatial_scale, sampling_ratio, output_dim_, group_size_);
}
...
template<>
Operator* CreateOp<gpu>(PSROIALIGNAVERotatedPoolingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new PSROIALIGNAVERotatedPoolingOp<gpu, DType>(param);
  });
  return op;
}
