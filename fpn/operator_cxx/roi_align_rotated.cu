  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {  // e.g., iy = 0, 1
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        T x = xx * cosTheta - yy * sinTheta + roi_center_w;
        T y = xx * sinTheta + yy * cosTheta + roi_center_h;
...
void ROIAlignRotatedForwardCompute(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(in_data[0].type_flag_, DType, {
    ...
    RoIAlignRotatedForwardKernel<DType>
...
.set_attr<FCompute>("FCompute<gpu>", ROIAlignRotatedForwardCompute<gpu>);

NNVM_REGISTER_OP(_backward_ROIAlignRotated)
.set_attr<FCompute>("FCompute<gpu>", ROIAlignRotatedBackwardCompute<gpu>);
